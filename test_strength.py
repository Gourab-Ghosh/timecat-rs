...
def play_game(
    ...
    while not (
        board.is_game_over() or board.is_fifty_moves() or board.is_repetition(3)
    ):
        ...
        if limit.time is not None:
            if time_taken > max(3 * limit.time, 0.1):
                print(
                    f"Engine {engine.index} exceeded time limit of {limit.time} s by {round(time_taken - limit.time, 3)} s with fen {board.fen()}"
                )
        move = analysis["pv"][0]
        if print_info:
            if engine1 is engine2:
                ...
                print(
                    f"Engine {board.san(move)} played {engine.index} in {round(time_taken, 3)} s"
                )
        board.push(move)
...
def get_stats_string(stats):
    return (
        f"Engine1 Win: {stats[True]}, Engine2 Win: {stats[False]}, Draw: {stats[None]}"
    )
...
def get_engine_paths():
    ...
    for engine_path in [FIRST_ENGINE_PATH, SECOND_ENGINE_PATH]:
        ...
        engine_new_path = os.path.join(
            temp_dir, hex(random.randint(start_num, end_num))[2:].upper() + engine_ext
        )
...
def main():
    ...
    random.seed(0)
    with Engine.popen_uci(engine_paths[0]) as engine1, Engine.popen_uci(
        engine_paths[1]
    ) as engine2:
        ...
        with open("test_fens.txt", "r") as rf:
            fens_and_opening_name = [
                get_fen_and_opening_name(line) for line in rf.readlines()
            ]
        ...
        for i, (fen, opening) in enumerate(fens_and_opening_name):
            try:
                start_time = time.time()
                outcome = play_game(
                    engine1, engine2, LIMIT, fen, "--verbose" in sys.argv, LOG_FILE_PATH
                )
                time_taken = time.time() - start_time
                ...
                print(
                    f"Game {i + 1} played in {round(time_taken, 3)} sec with opening {opening}, winner is {winner}, expected time left to finish: {time_left}"
                )
