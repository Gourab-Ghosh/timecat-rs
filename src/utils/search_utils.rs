...
impl SearchInfoBuilder {
    pub fn new(minimum_board: MinimumBoard, pv: Vec<Move>) -> Self {
        Self {
            minimum_board,
            pv,
            ..Default::default()
    ...
    pub fn set_minimum_board(mut self, minimum_board: MinimumBoard) -> Self {
    ...
    pub fn build(self) -> SearchInfo {
        SearchInfo {
            minimum_board: self.minimum_board,
...
impl SearchInfo {
    ...
    pub fn get_score_flipped(&self) -> Option<Score> {
        Some(self.minimum_board.score_flipped(self.get_score()?))
    }
    ...
    pub fn print_info(&self) {
        ...
        let outputs = [
            ...
            Self::format_info("pv", Some(get_pv_string(&self.minimum_board, &self.pv))),
        ];
    ...
    pub fn print_warning_message(&self, mut alpha: Score, mut beta: Score) {
        if GLOBAL_TIMECAT_STATE.is_in_console_mode() {
            alpha = self.minimum_board.score_flipped(alpha);
            beta = self.minimum_board.score_flipped(beta);
        }
...
impl<P: PositionEvaluation> From<&Searcher<P>> for SearchInfo {
    fn from(searcher: &Searcher<P>) -> Self {
        ...
        let mut search_info = Self {
            minimum_board: searcher.get_initial_minimum_board().to_owned(),
        ...
        search_info.score = search_info
            .score
            .map(|score| search_info.minimum_board.score_flipped(score));
        search_info
    }
}
