...
pub struct SpinValue_<T: Clone + Copy + IntoSpin> {
    default: T,
    min: T,
    max: T,
}

impl<T: Clone + Copy + IntoSpin> SpinValue_<T> {
    #[inline(always)]
...
pub enum UCIOption_ {
    Thread(SpinValue_<usize>),
    Hash(SpinValue_<CacheTableSize>)
}

pub struct UCIOptions_ {
    options: Vec<UCIOption_>
}
