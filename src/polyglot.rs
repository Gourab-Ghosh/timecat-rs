...
pub fn get_polyglot_piece_value(piece: Piece) -> usize {
    2 * piece.get_piece_type().to_index() + (1 - piece.get_color().to_index())
}
...
pub fn polyglot_hash_from_board(board: &Board) -> u64 {
    ...
    for rook_square in board.clean_castling_rights() {
        let offset = 768;
        let index = match rook_square {
            Square::H1 => 0,
            Square::A1 => 1,
            Square::H8 => 2,
            Square::A8 => 3,
    ...
    if let Some(ep_square) = board.ep_square() {
        let offset = 772;
        let index = ep_square.get_file().to_index();
        hash ^= POLYGLOT_RANDOM_64[offset + index];
    }
    if board.turn() == White {
        hash ^= POLYGLOT_RANDOM_64[780];
    }
    hash
}
