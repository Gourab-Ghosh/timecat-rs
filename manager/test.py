...
def test_package(release_mode=False):
    ...
    if release_mode:
        return bool(
            run_command_with_target_cpu_native(
                f"cargo test --no-default-features --features {features} --release"
            )
        )
    else:
        return bool(
            os.system(f"cargo test --no-default-features --features {features}")
        )
