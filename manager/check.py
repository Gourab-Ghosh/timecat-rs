...
def check_errors(feature_sets_check, clippy=False, verbose=True) -> bool:
    feature_sets_check = set(
        tuple(sorted(set(feature_set))) for feature_set in feature_sets_check
    )
    feature_sets_check.add(None)
    feature_sets_check = sorted(
        feature_sets_check, key=lambda k: (-1 if k is None else len(k), k)
    )
    if verbose:
    ...
    try:
        ...
        for feature_set in tqdm(
            feature_sets_check, desc="Checking Feature Combinations", leave=False
        ):
